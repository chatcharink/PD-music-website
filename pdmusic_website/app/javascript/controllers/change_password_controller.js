...
export default class extends Controller {
    ...
    validateNewPassword(){
        ...
        if (parseInt(password_valid.value) == 1 && (password.value == confirm_password.value)){
            this.valid_value(password);
            this.valid_value(confirm_password);
            document.getElementById("submit-change-password").click(); 
        } else if (parseInt(password_valid.value) == 0){
            // clear password
            this.invalid_value(password, true);
            this.invalid_value(confirm_password);
            this.alert("error", "Password format is incorrect. Please try again");
        }
        else {
            // clear password
            this.invalid_value(password, true);
            this.invalid_value(confirm_password);
            this.alert("error", "Confirm password doesn't not match. Please try again");
        }
    }
